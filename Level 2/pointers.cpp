...
void pointerPalindrome() {
	...
	char size = 0;
	for (int i = 0; i < 100; i++) {
		char currChar = (char)tolower(getchar());
		if (currChar == '\n') break;
		*(p + size) = currChar;
		size++;
	}
...
void showList(int* arr, int size) {
	for (int i = 0; i < size; i++) {
		printf("list + %d: ", i);
		for (ll j = sizeof(arr); j > 0; j--) {
			printf("%.2X ", (unsigned char)((ll)arr / (ll)pow(256, j - 1) % 256));
		}
		printf("contains %d\n", *(arr++));
	}
}
...
void multiplyArr() {
	...
	for (int i = 0; i < m; i++) {
		for (int j = 0; j < o; j++) {
			for (int k = 0; k < n; k++) {
				*(pm3 + (ll)i * o + j) += *(pm1 + (ll)i * n + k) * *(pm2 + (ll)k * o + j);
			}
		}
	}
...
void populateArr() {
	...
	printf("Convert to int*, pointing to an int.\n");
	printf("Step by columns until at right column.\n");
	printf("Dereference pointer.\n");
	printf("Read and print element.\n");
